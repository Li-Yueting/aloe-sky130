...
def construct():
  ...
  parameters = {
    ...
    'adk_view'       : adk_view,
    'topographical'  : True,
    'testbench_name' : 'GcdUnitWrapperTb',
    'saif_instance'  : 'GcdUnitWrapperTb/GcdUnitWrapper_inst',
    'dut_name'       : 'macro_inst'
  }
  ...
  # rtl_sim.extend_inputs([this_dir+'macro/sky130_macro_1kbyte_1rw1r_32x256_8.v'])
  # gl_sim.extend_inputs(['sky130_macro_1kbyte_1rw1r_32x256_8.v'])
  dc.extend_inputs(['macro.db'])
  dc.extend_inputs(['macro.lef'])
  pt_timing.extend_inputs(['macro.db'])
  pt_power_rtl.extend_inputs(['macro.db'])
  pt_power_gl.extend_inputs(['macro.db'])
  gdsmerge.extend_inputs(['macro.gds'])
  # netgen_lvs_def.extend_inputs(['sky130_macro_1kbyte_1rw1r_32x256_8.sp'])
  # netgen_lvs_gds.extend_inputs(['sky130_macro_1kbyte_1rw1r_32x256_8.sp'])
  calibre_lvs.extend_inputs(['macro.sp'])
  magic_drc.extend_inputs(['macro.lef'])
  for step in [iflow, init, power, place, cts, postcts_hold, route, postroute, signoff]:
    step.extend_inputs(['macro.lib', 'macro.lef'])
  ...
  # g.connect_by_name( macro,            rtl_sim         )
  # g.connect_by_name( macro,            gl_sim          )
  g.connect_by_name( macro,            dc              )
  g.connect_by_name( macro,            iflow           )
  g.connect_by_name( macro,            init            )
  g.connect_by_name( macro,            power           )
  g.connect_by_name( macro,            place           )
  # g.connect_by_name( macro,            cts             )
  # g.connect_by_name( macro,            postcts_hold    )
  g.connect_by_name( macro,            route           )
  g.connect_by_name( macro,            postroute       )
  g.connect_by_name( macro,            signoff         )
  g.connect_by_name( macro,            gdsmerge        )
  # g.connect_by_name( macro,            pt_timing       )
  # g.connect_by_name( macro,            pt_power_rtl    )
  # g.connect_by_name( macro,            pt_power_gl     )
  g.connect_by_name( macro,            magic_def2spice )
  g.connect_by_name( macro,            magic_gds2spice )
  g.connect_by_name( macro,            netgen_lvs_def  )
  g.connect_by_name( macro,            netgen_lvs_gds  )
  g.connect_by_name( macro,            calibre_lvs     )
  # g.connect_by_name( macro,            calibre_lvs_nobbox     )
  g.connect_by_name( macro,            magic_drc       )
