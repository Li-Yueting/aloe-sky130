...
streamOut $env(gds_dir)/bgr-v0.gds.gz \
   -units 1000 \
...
streamOut $env(gds_dir)/bgr-v0-merged.gds \
   -units 1000 \
   -mapFile $env(stemcell_dir)/rtk-stream-out.map \
   -merge $merge_files

# Write netlist for LVS check
saveNetlist -excludeLeafCell -flat -phys $env(lvs_dir)/bgr-v0.lvs.v
# Save DEF for use in running DC again
defOut -routing $env(def_dir)/bgr-v0.def.gz
